...
import aloqaAppIcon from "@/assets/aloqa-app-icon.svg.asset.json";
import {
  ArrowLeft,
  ArrowRight,
  ChevronRight,
...
function AloqaLogo({ className }: { className?: string }) {
  return (
    <div className={cn("bg-card grid place-items-center overflow-hidden", className)}>
      <img
        src={aloqaAppIcon.url}
        alt="aloQa"
        className="w-[86%] h-[86%] object-contain select-none"
        draggable={false}
      />
    </div>
  );
  ...
  return (
    <div className="space-y-5 px-5 pt-1">
      ...
      <div className="relative flex min-h-[92px] items-center gap-4 overflow-hidden rounded-[28px] border border-border bg-card px-6 py-5 shadow-sm shadow-foreground/5">
        {/* Ornament: only the top-left quarter of the medallion, fully inside the card */}
        <div className="pointer-events-none absolute right-0 top-0 h-[88px] w-[88px] select-none overflow-hidden" aria-hidden="true">
          <img
            src={balanceCardOrnament.url}
            alt=""
            className="absolute left-0 top-0 h-[176px] w-[176px] opacity-[0.055]"
          />
        </div>
  ...
  return (
    <div className="relative isolate overflow-hidden px-4 pt-2 pb-8 space-y-3">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-surface p-4 text-primary-foreground">
        <div className="absolute -right-5 -top-5 h-20 w-20 rounded-full bg-brand/25" />
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {perks.map((p) => (
          <div key={p.title} className="min-h-32 rounded-2xl bg-card border border-border p-3.5 shadow-sm shadow-foreground/5">
            <div className="grid h-9 w-9 place-items-center rounded-xl bg-brand/10 text-brand">
              <p.icon className="h-4 w-4" />
            </div>
            <div className="mt-3 text-xs font-extrabold leading-tight">{p.title}</div>
            <div className="mt-1 text-[10px] leading-relaxed text-muted-foreground">{p.text}</div>
          </div>
        ))}
      </div>

      <div className="rounded-2xl bg-card border border-border p-4 shadow-sm shadow-foreground/5">
        ...
        <Button
          ...
          onClick={onGo}
          className="mt-4 h-12 w-full rounded-xl bg-brand text-sm font-black text-brand-foreground shadow-none active:scale-[0.98]"
